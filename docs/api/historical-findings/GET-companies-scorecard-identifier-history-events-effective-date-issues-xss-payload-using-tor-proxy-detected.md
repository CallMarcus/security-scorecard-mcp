...
- **operationId:** `get_companies-scorecard-identifier-history-events-effective-date-issues-xss-payload-using-tor-proxy-detected`
...
```bash
curl -X GET \
  'https://api.securityscorecard.io//companies/<scorecard_identifier>/history/events/<effective_date>/issues/xss_payload_using_tor_proxy_detected/' \

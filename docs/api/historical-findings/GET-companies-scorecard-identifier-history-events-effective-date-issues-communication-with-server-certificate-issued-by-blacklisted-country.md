...
- **operationId:** `get_companies-scorecard-identifier-history-events-effective-date-issues-communication-with-server-certificate-issued-by-blacklisted-country`
...
```bash
curl -X GET \
  'https://api.securityscorecard.io//companies/<scorecard_identifier>/history/events/<effective_date>/issues/communication_with_server_certificate_issued_by_blacklisted_country/' \

...
- **operationId:** `get_companies-scorecard-identifier-history-events-effective-date-issues-local-file-path-exposed-via-url-scheme`
...
```bash
curl -X GET \
  'https://api.securityscorecard.io//companies/<scorecard_identifier>/history/events/<effective_date>/issues/local_file_path_exposed_via_url_scheme/' \

...
- **operationId:** `get_companies-scorecard-identifier-history-events-effective-date-issues-communication-server-with-expired-cert`
...
```bash
curl -X GET \
  'https://api.securityscorecard.io//companies/<scorecard_identifier>/history/events/<effective_date>/issues/communication_server_with_expired_cert/' \

...
- **Operation ID:** `get_companies-scorecard-identifier-history-events`
...
- `date_from` (string, Optional) - created at from
- `date_to` (string, Optional) - created at to
- `score_type` (string, Optional) - No description
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/history/events/' \

...
- **Method:** `DELETE`
- **Path:** `/custom-scorecards/{id}/filters`
- **Tag:** `custom scorecards`
- **operationId:** `delete_custom-scorecards-id-filters`

## Description
Delete custom scorecard filters

## Path Parameters
- `id` (**required**) — id of custom scorecard to delete filters
...
```bash
curl -X DELETE \
  'https://api.securityscorecard.io//custom-scorecards/<id>/filters' \

...
- **Tag:** `active findings`
- **operationId:** `get_companies-scorecard-identifier-issues-communication-with-server-certificate-issued-by-blacklisted-country`
...
```bash
curl -X GET \
  'https://api.securityscorecard.io//companies/<scorecard_identifier>/issues/communication_with_server_certificate_issued_by_blacklisted_country' \

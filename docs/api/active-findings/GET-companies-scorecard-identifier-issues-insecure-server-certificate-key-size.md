...
- **Tag:** `active findings`
- **operationId:** `get_companies-scorecard-identifier-issues-insecure-server-certificate-key-size`
...
```bash
curl -X GET \
  'https://api.securityscorecard.io//companies/<scorecard_identifier>/issues/insecure_server_certificate_key_size' \

...
- **Tag:** `{Parent Domain}`
- **operationId:** `getByParentdomainAttributionLog`

## Path Parameters
- `parentDomain` (**required**) — parent domain

## Query Parameters
- `date-from` (optional, string) — start date time range
- `date-to` (optional, string) — end date time range
- `sort-field` (optional, string) — sort field
- `filters` (optional, string) — filters field
- `filter-operator` (optional, string) — filter operation, expected values: and | or
- `page` (optional, number) — required page number, first page is 0
- `page-size` (optional, number) — size of the pages in the paginated result
...
```bash
curl -X GET \
  'https://api.securityscorecard.io//footprint/<parentDomain>/attribution-log' \

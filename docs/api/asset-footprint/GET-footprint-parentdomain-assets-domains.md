...
- **Operation ID:** `getByParentdomainAssetsDomains`
...
- `page-size` (integer, Optional) - Number of items per page, max 100
- `sort` (string, Optional) - Sort field, prefix with - for descending order
- `filters` (string, Optional) - Filter string in format: "field:condition:value,field2:condition2:value2"
- `filter-operator` (string, Optional) - Filter operator, expected values: and | or
- `include-evidence` (boolean, Optional) - Include evidence sources in response
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/footprint/<parentDomain>/assets/domains' \

...
- **Method:** `GET`
- **Path:** `/v2/portfolios/{id}/companies`
- **Tag:** `Portfolio`
- **operationId:** `get_v2-portfolios-id-companies`
...
## Query Parameters
- `with_factors` (optional, boolean) — return factors, along with portfolio companies
...
- `custom_fields` (optional, string) — filter by custom VSOR field values — URL-encoded JSON `{ fieldId: [values] }`
...
```bash
curl -X GET \
  'https://api.securityscorecard.io//v2/portfolios/<id>/companies' \

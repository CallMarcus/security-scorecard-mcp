...
- **Tag:** `User`
- **operationId:** `get_users-by-username-username-notifications-recent`
...
## Query Parameters
- `portfolio` (optional, string) — filter notifications by portfolio id
- `sort` (optional, string) — sort notifications by unread status or date
- `order` (optional, string) — specify the order of sorting, asc or desc
- `unread` (optional, boolean) — Wether to filter by read or unread notifications
...
```bash
curl -X GET \
  'https://api.securityscorecard.io//users/by-username/<username>/notifications/recent' \

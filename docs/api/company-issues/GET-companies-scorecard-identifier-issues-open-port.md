...
- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/issues/open_port`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-issues-open-port`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "description": "\"open_port\" issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of OpenPorts"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/issues/open_port' \

# Get "websocket_sends_data" historical issues in a 

- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/history/events/{effective_date}/issues/websocket_sends_data/`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-history-events-effective-date-issues-websocket-sends-data`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "additionalProperties": false,
        "description": "\"websocket_sends_data\" historical issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of HistoricalWebsocketSendsData"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/history/events/<effective_date>/issues/websocket_sends_data/' \

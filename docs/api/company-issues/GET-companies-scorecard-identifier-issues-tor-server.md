...
- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/issues/tor_server`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-issues-tor-server`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "description": "\"tor_server\" issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of TorServers"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/issues/tor_server' \

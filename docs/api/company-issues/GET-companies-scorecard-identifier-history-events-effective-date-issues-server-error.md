# Get "server_error" historical issues in a scorecar

- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/history/events/{effective_date}/issues/server_error/`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-history-events-effective-date-issues-server-error`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "additionalProperties": false,
        "description": "\"server_error\" historical issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of HistoricalServerErrors"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/history/events/<effective_date>/issues/server_error/' \

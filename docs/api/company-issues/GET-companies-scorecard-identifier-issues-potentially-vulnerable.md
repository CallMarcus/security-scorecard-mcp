...
- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/issues/potentially_vulnerable`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-issues-potentially-vulnerable`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "description": "\"potentially_vulnerable\" issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of PotentiallyVulnerables"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/issues/potentially_vulnerable' \

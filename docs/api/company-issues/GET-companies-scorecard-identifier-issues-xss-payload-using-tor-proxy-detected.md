# Get "xss_payload_using_tor_proxy_detected" issues 

- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/issues/xss_payload_using_tor_proxy_detected`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-issues-xss-payload-using-tor-proxy-detected`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "description": "\"xss_payload_using_tor_proxy_detected\" issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of XssPayloadUsingTorProxyDetecteds"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/issues/xss_payload_using_tor_proxy_detected' \

# Get "insecure_server_certificate_key_size" histori

- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/history/events/{effective_date}/issues/insecure_server_certificate_key_size/`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-history-events-effective-date-issues-insecure-server-certificate-key-size`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "additionalProperties": false,
        "description": "\"insecure_server_certificate_key_size\" historical issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of HistoricalInsecureServerCertificateKeySizes"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/history/events/<effective_date>/issues/insecure_server_certificate_key_size/' \

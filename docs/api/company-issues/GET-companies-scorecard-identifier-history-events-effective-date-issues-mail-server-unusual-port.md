# Get "mail_server_unusual_port" historical issues i

- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/history/events/{effective_date}/issues/mail_server_unusual_port/`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-history-events-effective-date-issues-mail-server-unusual-port`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "additionalProperties": false,
        "description": "\"mail_server_unusual_port\" historical issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of HistoricalMailServerUnusualPorts"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/history/events/<effective_date>/issues/mail_server_unusual_port/' \

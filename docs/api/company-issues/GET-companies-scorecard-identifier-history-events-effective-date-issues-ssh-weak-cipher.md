# Get "ssh_weak_cipher" historical issues in a score

- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/history/events/{effective_date}/issues/ssh_weak_cipher/`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-history-events-effective-date-issues-ssh-weak-cipher`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "additionalProperties": false,
        "description": "\"ssh_weak_cipher\" historical issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of HistoricalSshWeakCiphers"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/history/events/<effective_date>/issues/ssh_weak_cipher/' \

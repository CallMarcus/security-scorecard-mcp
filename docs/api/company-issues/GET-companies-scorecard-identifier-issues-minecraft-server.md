...
- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/issues/minecraft_server`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-issues-minecraft-server`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "description": "\"minecraft_server\" issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of MinecraftServers"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/issues/minecraft_server' \

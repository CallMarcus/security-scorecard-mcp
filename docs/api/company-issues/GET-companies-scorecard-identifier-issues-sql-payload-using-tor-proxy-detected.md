# Get "sql_payload_using_tor_proxy_detected" issues 

- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/issues/sql_payload_using_tor_proxy_detected`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-issues-sql-payload-using-tor-proxy-detected`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "description": "\"sql_payload_using_tor_proxy_detected\" issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of SqlPayloadUsingTorProxyDetecteds"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/issues/sql_payload_using_tor_proxy_detected' \

...
- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/issues/payment_provider`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-issues-payment-provider`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "description": "\"payment_provider\" issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of PaymentProviders"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/issues/payment_provider' \

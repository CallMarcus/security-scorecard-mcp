...
- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/issues/exposed_printer`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-issues-exposed-printer`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "description": "\"exposed_printer\" issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of ExposedPrinters"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/issues/exposed_printer' \

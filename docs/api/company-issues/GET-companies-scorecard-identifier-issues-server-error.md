...
- **Method:** `GET`
- **Path:** `/companies/{scorecard_identifier}/issues/server_error`
- **Category:** `company-issues`
- **Operation ID:** `get_companies-scorecard-identifier-issues-server-error`
...
```json
{
  ...
  "properties": {
    "entries": {
      ...
      "items": {
        ...
        "description": "\"server_error\" issues in a scorecard"
      }
    }
  },
  "additionalProperties": false,
  ...
  "description": "A page in a list of ServerErrors"
}
```
...
```bash
curl -X GET \
  'https://platform.securityscorecard.io/companies/<scorecard_identifier>/issues/server_error' \
